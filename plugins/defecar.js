...
export async function run(sock, msg, args) {
  ...
  if (!mencionado || mencionado === sender) {
    await sock.sendMessage(from, {
      text: '💩 Debes mencionar a alguien para defecar sobre él.'
    }, { quoted: msg });
    return;
  }
  ...
  const texto = `💩 *Defecación ejecutada*\n\n@${senderId} le defecó encima a @${mencionado.split('@')[0]} y le robó *${monto.toLocaleString()} pandacoins* sin piedad.`;
