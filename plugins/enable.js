...
export async function run(sock, msg, args) {
  ...
  if (!option) return sock.sendMessage(from, { text: '⚙️ Escribe qué función quieres habilitar: antilink / modoadmin / modoowner / grupos / chatsprivados' }, { quoted: msg });

  const config = JSON.parse(fs.readFileSync('./config.json', 'utf8'));

  // Si el comando viene de un grupo
  if (isGroup) {
    ...
    if (option === 'antilink' || option === 'modoadmin') {
      config.groups[from][option] = true;
    } else {
      return sock.sendMessage(from, { text: '❌ Esa opción solo puede activarse desde el chat privado del owner.' }, { quoted: msg });
    }

  } else {
    // Si el comando viene de un chat privado
    ...
    if (option === 'modoowner' || option === 'grupos' || option === 'chatsprivados') {
      config.global[option] = true;
    } else {
      return sock.sendMessage(from, { text: '❌ Esa opción solo puede activarse desde un grupo.' }, { quoted: msg });
    }
  }

  fs.writeFileSync('./config.json', JSON.stringify(config, null, 2));
  await sock.sendMessage(from, { text: `✅ Función *${option}* habilitada.` }, { quoted: msg });
}
