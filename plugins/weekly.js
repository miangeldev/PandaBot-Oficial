...
export async function run(sock, msg, args) {
  ...
  const last = user.cooldowns.weekly || 0;
  ...
  const coins = 700 + Math.floor(Math.random() * 800);
  const exp = 10000 + Math.floor(Math.random() * 3000);
  ...
  await sock.sendMessage(from, { text: `🗓️ ¡Recompensa semanal!\n+${coins} 🪙 Pandacoins\n+${exp} ⭐ Experiencia` });
}
