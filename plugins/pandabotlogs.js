...
export async function run(sock, msg, args) {                                                                const from = msg.key.remoteJid;
  ...
  const loadingMsg = await sock.sendMessage(from, { text: '⏳ Obteniendo los logs de PandaBot...' });

  try {
    const { stdout, stderr } = await execPromise('pm2 logs --nostream bot --lines 50');
    ...
    if (stdout) {
      await sock.sendMessage(from, { text: `📜 *Logs de PandaBot:*\n\n\`\`\`\n${stdout}\n\`\`\`` });
    } else {
